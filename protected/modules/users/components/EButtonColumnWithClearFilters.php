...
*/
    ...
    public function init()
    {
        ...
        //define defaults
        $_optionsDefault=array('class'=>'btn btn-default clear','title'=>'Очистить фильтр','style'=>'text-align:center; margin: 0 auto;');
        ...
        $_click="js:function() {{$_customJS} return cbcwr_clearFields('".$this->grid->id."') }";
        ...
        if (!empty($this->label)) {
           ...
           // $_label=Yii::t('app','Очистить фильтр');
            $_label='<span class="glyphicon glyphicon-filter" aria-hidden="true"></span>';
...
function cbcwr_clearFields(id) {
    try
    {
        $('#' + id + ' :input').clearFields(); // this will clear all input in the current grid
        {$_beforeAjax} $('#' + id + ' :input').first().trigger('change');// to submit the form
        return false;
    }
    catch(cbwr_err)
    {
